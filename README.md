...
- Use `task_dispatcher.dispatch(f, args...)` to dispatch new tasks
...
- Use `task_dispatcher.parallel_for(f, begin, end, batch_size)` or `task_dispatcher.parallel_for(f, range, batch_size)` to process ranges in parallel
